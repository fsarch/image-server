...
export class SignedUrlService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.signedUrlsConfig =
      this.configService.get<ConfigSignedUrlsType>('signed_urls');
  }
   ...
   */
  generateSignedUrl(path: string, options: GenerateSignedUrlOptions): string {
    if (!this.isEnabled) {
  ...
  validateRequest(req: Request): SignedUrlValidationResult {
    ...
    const query = req.query as unknown as Record<
      string,
      string | Array<string>
    >;
    ...
    const payload = buildSignaturePayload(req.method, req.path, query);
