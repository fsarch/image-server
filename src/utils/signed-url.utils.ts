...
export function buildSignaturePayload(
  ...
  for (const [key, value] of Object.entries(query)) {
    ...
    // Handle array values by encoding each item and joining with a literal comma
    if (Array.isArray(value)) {
      filteredQuery[key] = value.map((v) => encodeURIComponent(v)).join(',');
    } else {
      filteredQuery[key] = encodeURIComponent(value);
    }
  }
  ...
  for (const key of sortedKeys) {
    ...
    queryStringParts.push(`${encodedKey}=${filteredQuery[key]}`);
  }
...
export function sign(payload: string, key: ConfigSignedUrlKeyType): string {
  ...
  if (isEd25519Key(key)) {
    ...
    try {
      ...
      console.warn(
        'ED25519 signing failed, falling back to HMAC-SHA256:',
        error,
      );
      return crypto
        .createHmac('sha256', key.secret)
...
export function verify(
  ...
  if (isEd25519Key(key)) {
    ...
    try {
      ...
      console.warn(
        'ED25519 verification failed, attempting HMAC fallback:',
        error,
      );
      const expectedSignature = crypto
        .createHmac('sha256', key.secret)
  ...
  throw new Error(
    `Unsupported verification algorithm: ${(key as any).algorithm}`,
  );
}
