...
import type {
  ConfigCachingClientType,
  ConfigNamingType,
} from '../types/config.type.js';
import {
  getFormatInfoByExtension,
  getFormatInfoByMimeType,
} from '../utils/format-info.utils.js';
import { runInBackground } from '../utils/run-in-background.utils.js';
import { ImageService, type ResolveResponseType } from './image.service.js';
...
export class ImageController {
  ...
  private async getBaseImage(
    ...
    const clientCachingOptions =
      this.configService.get<ConfigCachingClientType>('caching.client.options');
    ...
    if (resolveInfo.imageCaches?.[0]?.path) {
      ...
      try {
        const cachedData = await this.cacheService.getOrCreateCache(
            ...
            return {
              ...
              meta: {
                ...
                imageCache: {
                  ...
                  imagePresetId:
                    resolveInfo.imageCaches[0].value.imagePreset.id,
          ...
          {
            calculateSize: (value) =>
              value.buffer.length + JSON.stringify(value.meta).length,
          },
        );
        ...
        if (error.code === 'ENOENT' || error.name === 'NoSuchKey') {
          ...
          runInBackground(async () => {
            await this.imageService
              ...
              .catch((ex) =>
                console.error(
                  'error while removing cached image',
                  ex,
                  cacheInfo,
                ),
              );
    ...
    if (resolveInfo.preset.cached) {
      await this.imageService.saveCached(
        resolveInfo.image,
        convertedImage,
        resolveInfo.preset.alias,
  ...
  @ApiQuery({
    name: 'x-expires',
    description:
      'Expiration timestamp for signed URL (milliseconds since epoch)',
    required: false,
    schema: { type: 'string' },
  })
  @ApiQuery({
  ...
  public async getImageById(
    ...
    const preferredMimeTypes = headers.accept
      .split(',')
    ...
    const resolveInfo = await this.cacheService.getOrCreateCache(
  ...
  public async getImage(
    ...
    const preferredMimeTypes = ext
      ? [getFormatInfoByExtension(ext).mimeType]
      : headers.accept.split(',').map((a) => a.split(';')[0].trim());
