...
import type { ImagePreset } from '../database/entities/image-preset.entity.js';
...
import type { ConfigImagePresetType } from '../types/config.type.js';
...
export class ImageService {
  ...
  async resolveImage(options: {
    ...
    const presets =
      this.configService.get<Array<ConfigImagePresetType>>('images.presets');
    ...
    const sortedCaches = imageCache
      .map((imageCache) => ({
        value: imageCache,
        priority:
          options.preferredMimeTypes.length -
          options.preferredMimeTypes.findIndex(
            (mimeType) => mimeType === imageCache.mimeType,
          ),
        path: this.getImageCacheFile(
          image.id,
          image.creationTime,
          imageCache.creationTime,
          options.presetAlias,
          imageCache.mimeType,
        ),
      }))
    ...
    return {
      imagePath: path.join(
        this.getImageDirectory(image.creationTime),
  ...
  async saveCached(
    ...
    const path = this.getImageCacheFile(
      image.id,
      image.creationTime,
      cacheCreationTime,
      alias,
    ...
    await this.imageCachesRepository.save({
      ...
      md5: crypto.createHash('md5').update(convertedImage).digest('hex'),
